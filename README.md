...
- `review_report.log` in the current directory keeps the human review report section for compatibility, and each run also writes scoped artifacts under `.crucible/runs/<run_id>/`.
- `.crucible/runs/<run_id>/progress.log` is the human-readable progress stream with timestamps, phase changes, role summaries, timing summaries, and final report sections.
